...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_LBUTTONDOWN:
		//hdc = GetDC(hwnd);
		isClick = true;
		if (0 != FigureCount && NULL != inform[FigureCount]) {
	...
	case WM_PAINT:
		...
		for (int i = 1; i < FigureCount; i++)
		{
			...
			Line line(sX, sY, eX, eY, object[i].kind);
			Ellipse1 el(sX, sY, eX, eY, object[i].kind);
			Rectangle1 rect(sX, sY, eX, eY, object[i].kind);
