...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_PAINT:
		...
		for (int i = 0; i <= FigureCount; i++)
		{
			...
			Line line(inform[i]->startX, inform[i]->startY, inform[i]->endX, inform[i]->endY, inform[i]->kind);
			Ellipse1 el(inform[i]->startX, inform[i]->startY, inform[i]->endX, inform[i]->endY, inform[i]->kind);
			Rectangle1 rect(inform[i]->startX, inform[i]->startY, inform[i]->endX, inform[i]->endY, inform[i]->kind);
